...
Namespace Settings
    Public Class UserStateFluxRead
        Inherits SettingsGroup

        Private ReadOnly _Devices As Dictionary(Of IDevice.FluxDevice, UserStateFluxReadDevice)
        Private _LastDevice As IDevice.FluxDevice?

        Public Sub New()
            _Devices = New Dictionary(Of IDevice.FluxDevice, UserStateFluxReadDevice)
            For Each dev In [Enum].GetValues(GetType(IDevice.FluxDevice))
                If (FluxDeviceGetCapabilities(CType(dev, IDevice.FluxDevice)) And DeviceCapabilities.Read) = 0 Then
                    Continue For
                End If
                _Devices(CType(dev, IDevice.FluxDevice)) = New UserStateFluxReadDevice()
            Next
        End Sub
        ...
        Friend ReadOnly Property Device(DeviceId As IDevice.FluxDevice) As UserStateFluxReadDevice
            Get
            ...
            If DeviceSection IsNot Nothing Then
                For Each kvp In DeviceSection
                    ...
                    If [Enum].TryParse(DeviceName, ignoreCase:=True, result:=DeviceEnum) Then
                        Dim DeviceSettings As UserStateFluxReadDevice = Nothing
